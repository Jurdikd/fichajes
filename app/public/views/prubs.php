<?php
# Comprueba si la tabla tiempo_fichaje ya existe
$tableExists = false;
try {
    $sql = "SELECT 1 FROM tiempo_fichaje LIMIT 1";
    Conexion::obtener_conexion()->query($sql);
    $tableExists = true;
} catch (PDOException $ex) {
    // La tabla no existe, se captura la excepción pero no se realiza ninguna acción
}

if ($tableExists) {
    echo "La tabla tiempo_fichaje ya existe.";
} else {
    # Crea la tabla tiempo_fichaje
    try {
        $sql = "CREATE TABLE tiempo_fichaje (
            id_tiempo_fichaje INT(6) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
            fecha_open TIMESTAMP NULL,
            fecha_cierre TIMESTAMP NULL
        )";

        Conexion::obtener_conexion()->exec($sql);
        echo "La tabla tiempo_fichaje se creó correctamente.";
    } catch (PDOException $ex) {
        echo "Error al crear la tabla tiempo_fichaje: " . $ex->getMessage();
    }
}
?>
<!DOCTYPE html>
